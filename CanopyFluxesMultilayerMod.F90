module CanopyFluxesMultilayerMod
  ...
  mlcanopy_inst,urbanparams_inst,soil_water_retention_curve,canopystate_inst)
    ...
    use SoilMoistStressMod    , only : calc_effective_soilporosity, calc_volumetric_h2oliq ! jsong
    use SoilMoistStressMod    , only : calc_root_moist_stress, set_perchroot_opt ! jsong
    !use clm_time_manager,  only : get_curr_calday ! jsong
...
!write (iulog,*) 'num_exposedvegp=',num_exposedvegp, bounds%endp, bounds%endc, bounds%endg
 ...
 !       dayl_factor(p)=min(1._r8,max(0.01_r8,(dayl(g)*dayl(g))/(max_dayl(g)*max_dayl(g))))
        ...
        htop_patch(p) = 43.725_r8 ! maximum canopy height between two trees by slope effect -  htop+H*(1-r) = 39+13.5*(1-0.65)

		!bbf =      int(ceiling(nlevcanml-ztop(p)/(zref(p)/nlevcanml))) ! nint
		!ntop        (p) = nlevcanml-bbf
		if (zref(p)+0.001_r8 < htop_patch(p)) then ! if forcing height is lower than canopy height
		    write(iulog,*) zref(p) , htop_patch(p)
		    call endrun (msg=' zref(p) < htop_patch(p)')
		end if
		if (abs(zref(p)-htop_patch(p))<nlevcanml/zref(p)) then ! select a grid for the top of canopy
		    ntop(p) = nlevcanml - 1
		else
		    ntop(p) = nint(htop_patch(p)*nlevcanml/zref(p)+0.4999_r8)
		end if
        nuforce(p) = nint(zuref*nlevcanml/zref(p)+0.4999_r8) ! * select a grid wind forcing height
        ...
        ztop(p) = zwinput(ntop(p)) ! this top of canopy would be somewhat different from an estimated value 'htop_patch(p)' because it is based on 'ntop'
        zuforce(p) = zwinput(nuforce(p)) ! this one too  [wind forcing height]

        ztop39(p) = 39._r8 ! average canopy height is 39m

        ! vertical leaf shape parameters (Beta Distribution)
        ...
        dlaiinput1(:) = 0.0_r8 ! buffer
        dpaiinput1(:) = 0.0_r8 ! buffer
        dlaiinput2(:) = 0.0_r8 ! buffer
        dpaiinput2(:) = 0.0_r8 ! buffer
        ...
        ! rescaling and adding a grid for zero index
