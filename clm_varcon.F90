module clm_varcon
  ...
  ! jsong
  ! CanopyTurbulence
  integer, parameter :: nZ=276
  integer, parameter :: nL=41
  integer, parameter :: intone = 1
  real(r8), dimension(intone,intone:nL)     ::  dtLgridM
  real(r8), dimension(intone:nZ,intone)     ::  zdtgridM
  real(r8), dimension(intone:nZ,intone:nL)  ::  psigridM
  real(r8), dimension(intone,intone:nL)     ::  dtLgridH
  real(r8), dimension(intone:nZ,intone)     ::  zdtgridH
  real(r8), dimension(intone:nZ,intone:nL)  ::  psigridH
  
  ! LeafBoundaryLayer

  real(r8), parameter :: visc0 = 15.0e-6_r8! Kinematic viscosity (m2/s)
  real(r8), parameter :: dh0 = 21.5e-6_r8! Molecular diffusivity, heat (m2/s)
  real(r8), parameter :: dv0 = 24.8e-6_r8! Molecular diffusivity, H2O (m2/s)
  real(r8), parameter :: dc0 = 14e-6_r8! Molecular diffusivity, CO2 (m2/s)
  real(r8), parameter :: cpbio = 1396._r8!cpbio * dry_weight + cpliq * leaf_water ! Heat capacity (J/K/m2 leaf)
  !real(r8), parameter :: cpliq = 4.188!cpbio * dry_weight + cpliq * leaf_water ! Heat capacity (J/K/m2 leaf) 
  
  ! CanopyFlux
  real(r8), parameter :: mmh2o=SHR_CONST_MWWV*0.001_r8
  real(r8), parameter :: mmdry=SHR_CONST_MWDAIR*0.001_r8
  real(r8), parameter :: cpd=1005 !j/kg/K Specific heat of dry air
  real(r8), parameter :: cpw=1864 !j/kg/K Specific heat of vapor
  real(r8), parameter :: rgasc=SHR_CONST_RGAS*0.001_r8
  ...
  real(r8), parameter :: n_melt=0.7_r8                         ! fsca shape parameter
  real(r8), parameter :: e_ice=6.0_r8                          ! soil ice impedance factor
  real(r8), parameter :: pc = 0.4_r8                           ! threshold probability
  real(r8), parameter :: mu = 0.13889_r8                       ! connectivity exponent
