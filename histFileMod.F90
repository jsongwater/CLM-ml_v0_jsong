...
!$OMP END PARALLEL DO
  ...
  subroutine htape_create (t, histrest)
    ...
    !
    ! !USES:
    use clm_varpar      , only : nlevgrnd, nlevsno, nlevlak, nlevurb, numrad, nlevcan, nvegwcs,nlevsoi, nlevcanml,nlevcanmll
    use clm_varpar      , only : natpft_size, cft_size, maxpatch_glcmec, nlevdecomp_full,nlevgrnd
    ...
    call ncd_defdim(lnfid, 'nlevcanml',nlevcanml, dimid)
    call ncd_defdim(lnfid, 'nlevcanmll',nlevcanmll, dimid)
    call ncd_defdim(lnfid, 'nlevgrnd',nlevgrnd, dimid)
...
!------------------------------------------------------------------------
  ...
  subroutine hist_addfld2d (fname, type2d, units, avgflag, long_name, type1d_out, &
    ...
    !
    ! !USES:
    use clm_varpar      , only : nlevgrnd, nlevsno, nlevlak, numrad, nlevdecomp_full, nlevcan, nvegwcs,nlevsoi,nlevcanml,nlevcanmll
    use clm_varpar      , only : natpft_size, cft_size, maxpatch_glcmec,nlevgrnd
    use landunit_varcon , only : max_lunit
    !
    ! !ARGUMENTS:
    character(len=*), intent(in) :: fname                      ! field name
