module clm_driver
  ...
  use CanopyFluxesMultilayerMod , only : CanopyFluxesMultilayer !jsong
...
!! jsong start
       ...
       call t_stopf('patch2col')
         ...
         call CanopyFluxesMultilayer (bounds_clump, filter(nc)%num_exposedvegp, filter(nc)%exposedvegp, &
  filter(nc)%num_nolakec, filter(nc)%nolakec,declin, atm2lnd_inst, temperature_inst, waterstate_inst, &
