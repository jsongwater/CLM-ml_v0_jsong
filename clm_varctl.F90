module clm_varctl
  ...
  ! jsong
  ! CanopyTurbulence
  integer, public, parameter :: allprofiles = 0  !  most+1st-order ==0 ,  1st order = 1
  integer, public, parameter :: incanopy = 0 ! most = 0, 1st order = 1
  integer, public, parameter :: turb_type = 1 ![0 4]
  integer, public :: gstyp = 1 ! water stress [0 2]
  real(r8), public :: dtime_sub = 300._r8 !5._r8 * 60._r8 !
  
  logical, public  :: use_scalars = .true. !
  logical, public  :: use_clm45kn = .false. ! Leaf nitrogen decay coefficient kn = 0.3_r8
  logical, public  :: use_acclim = .true. ! jmax25top = (2.59_r8 - 0.035_r8*min(max((tacclim(p)-tfrz),11._r8),35._r8)) * vcmax25top

  !If leaf_temp_iter = false, the leaf
  ...
  logical, public  :: leaf_temp_iter = .true. 
  ! Water storage of intercepted precipitation. Intercepted water
  ! is applied equally to all layers.
  logical, public  :: use_h2ocan = .true.
  logical, public  :: use_colim = .true. ! Net photosynthesis as the minimum or co-limited rate
  logical, public  :: no_storage = .true. ! Use very low leaf heat capacity to reduce storage term
  integer, public  :: light = 3 ![1 Norman 2 Goudriaan 3 TwoStream] Calculate radiative transfer through canopy
